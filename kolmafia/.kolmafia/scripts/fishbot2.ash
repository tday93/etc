...
void fish_equip_gear() {
	//Make sure we have any buyable fishing gear
	foreach it in $items[fishin' hat] {
		if (it.available_amount() < 1 && it.fish_item_price() < min(get_property("autoBuyPriceLimit").to_int(), my_meat()))
			buy(1, it);
	}
...
void keep_up_buffs() {
	...
	foreach i, r in maximize("fishing skill", 0, 2, true, false) {
		if (r.item.numeric_modifier("Effect Duration") * r.score / 100.0 * get_property("valueOfAdventure").to_float() >= r.item.fish_item_price())
